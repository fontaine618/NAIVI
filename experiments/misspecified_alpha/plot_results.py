...
experiments = {
    ...
    "misspecified_alpha": ("data.heterogeneity_variance", "data.heterogeneity_variance", "Heterogeneity variance", False),
}
seeds = range(30)
...
curves_by = "method"
cols_by = "data.heterogeneity_mean"
...
for name, (group_by, display_var, display_name, _) in experiments.items():
    ...
    pdf = pd.DataFrame(columns=[curves_by, rows_by, cols_by, group_by, "x_value",
...
# plots
plt.cla()
fig, axs = plt.subplots(figsize=(10, 5), nrows=len(rows)*2, ncols=len(cols),
        ...
        for _, curve in enumerate(curves):
            ...
            for name, (group_by, display_var, display_name, logx) in experiments.items():
                if col == display_name:
                    ax.set_xscale("log" if logx else "linear")
            if i == len(rows)-1:
                ax.set_xlabel("Heterogeneity variance")
            # ax.set_yscale("log")
            if j == 0:
