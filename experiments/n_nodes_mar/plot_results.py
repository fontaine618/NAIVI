...
experiments = {
    ...
    "n_nodes_mar": ("data.missing_covariate_rate", "data.missing_covariate_rate", "Diff. MAR Stength", True),
}
seeds = range(30)
...
curves_by = "method"
cols_by = ["data.n_nodes", "data.p_bin"]
...
# 2x2 experiment
cols_by = ["data.n_nodes", "data.p_bin"]
full_df["experiment"] = [f"N={n}, p={p}" for n, p in zip(full_df[cols_by[0]], full_df[cols_by[1]])]
full_pdf["experiment"] = [f"N={n}, p={p}" for n, p in zip(full_pdf[cols_by[0]], full_pdf[cols_by[1]])]
cols_by = "experiment"
...
# plots
plt.cla()
fig, axs = plt.subplots(figsize=(10, 6), nrows=len(rows)*2, ncols=len(cols),
