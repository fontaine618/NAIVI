...
# Plot
variables = [6, 9, 18, 27, 30, 36, 38, 39]
variables = [6, 27, 36, 38, 39]
nrows = len(models)
ncols = len(variables)
...
for row, (model_name, model) in enumerate(models.items()):
    ...
    for col, var in enumerate(variables):
        ...
        if row == 0:
            title = featnames[var]
            # remove underscore and captialize
        ...
        # add positions
        x = X_bin[:, var].float()
        c = ["#00274c" if xi == 0 else "#ffcb05" for xi in x]
...
plt.tight_layout()
plt.savefig("./experiments/facebook/position_plot_dissertation.png", bbox_inches="tight")
