...
from sims_mnar_size.post_processing import post_processing
from sims_mnar_size.run import run
...
def main(path, name, explore_dict):
    ...
    traj.f_add_parameter(
        "data.mnar_sparsity", np.float64(0.5), "Proportion of attributes with MCAR"
    )
    ...
    traj.f_add_parameter(
        "model.mnar", np.bool(True), "Whether to fit MNAR or not"
    )
    ...
    traj.f_add_parameter(
        "fit.max_iter", np.int64(100), "Number of VEM iterations"
    )
    traj.f_add_parameter(
