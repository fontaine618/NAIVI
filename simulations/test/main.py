...
def run(traj):
    ...
    traj.f_add_result("runs.$.missing_rate", missing_rate, "missing_rate")
    ...
    print(p_bin, missing_rate, seed, elbo, mse, auroc, dist_inv, dist_proj, density)
    return p_bin, missing_rate, seed, elbo, mse, auroc, dist_inv, dist_proj, density
...
def post_processing(traj, result_list):
    ...
    df = pd.DataFrame({
        ...
        "p_cts": [p_bin[i] for i in run_idx],
    ...
    df.to_csv(PATH + "simulations/test/results/summary.csv")
...
def main():
    ...
    env = Environment(
        trajectory="test",
        comment="Experiment on missing rate with binary covariates -- test",
        log_config=None,
        multiproc=False,
        ncores=1,
        filename=PATH + "simulations/test/results/",
        overwrite_file=True
    )
    traj = env.trajectory
